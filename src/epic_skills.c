...
int epic_teacher(P_char ch, P_char pl, int cmd, char *arg)
{
  ...
  if (!arg || !*arg)
  {
    ...
    if (GET_CHAR_SKILL(pl, skl) < 100)
    {
      ...
      if (!(pReward->classes && !IS_SET(pReward->classes, pl->player.m_class) && !IS_SET(pReward->classes, pl->player.secondary_class)) && !(IS_THRIKREEN(pl) && skl == SKILL_DEVASTATING_CRITICAL) && !(pTeacher->deny_skill && GET_CHAR_SKILL(pl, pTeacher->deny_skill)) && !(pTeacher->pre_requisite && GET_CHAR_SKILL(pl, pTeacher->pre_requisite) < pTeacher->pre_req_lvl) && !(GET_CHAR_SKILL(pl, skl) >= 100 || GET_CHAR_SKILL(pl, skl) >= pTeacher->max))
      {
...
int chant_mastery_bonus(P_char ch, int dura)
{
  ...
  if (5 + GET_CHAR_SKILL(ch, SKILL_CHANT_MASTERY) / 10 > number(0, 100))
  {
    ...
    snprintf(buffer, 256, "%s magic surrounds you as you begin your chant.&n",
             chant_bonus == 0 ? "&+WSparkling&n" : chant_bonus == 1 ? "&+WSparkling"
                                                                    : "&+WSp&+Cark&+Wli&+Cn&+Wg");
