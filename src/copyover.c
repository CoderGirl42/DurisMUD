...
{
    if (!d->websocket || d->descriptor < 0) return;
...
{
    if (!d->sslses || d->descriptor < 0) return;
...
void copyover_save(int mother_desc, int mother_desc_ssl, int ws_desc)
{
    ...
    for (d = descriptor_list; d; d = d_next) {
        ...
        logit(LOG_STATUS, "copyover: desc fd=%d state=%d char=%p ws=%d ssl=%d",
              d->descriptor, d->connected, (void*)d->character, d->websocket, d->sslses ? 1 : 0);
    ...
    logit(LOG_STATUS, "copyover: saved %d descs, %d mobs, %d objs, %d doors",
          num_descs, num_mobs, num_objs, num_rooms);

    // prepare listener sockets
    copyover_prepare_socket(mother_desc);
    copyover_prepare_socket(mother_desc_ssl);
    if (ws_desc > 0) copyover_prepare_socket(ws_desc);
...
void copyover_recover(int *mother_desc, int *mother_desc_ssl, int *ws_desc)
{
        ...
        if (desc_entry.player_name[0]) {
            ...
            if (ch) {
                    ...
                    if (pet_vnum > 0) {
                        ...
                        if (pet_rnum >= 0) {
                            ...
                            if (pet) {
                                ...
                                logit(LOG_STATUS, "copyover: restored pet %s for %s (%d/%d hp)",
                                      GET_NAME(pet), GET_NAME(ch), GET_HIT(pet), GET_MAX_HIT(pet));
                            }
                        }
                    }
                }
                ...
                logit(LOG_STATUS, "copyover: restored %s fd=%d room=%d fighting=%d",
                      desc_entry.player_name, d->descriptor, save_room, desc_entry.fighting_type);
            } else {
    ...
    for (i = 0; i < header.num_objects; i++) {
        ...
        if (fread(&obj_entry, sizeof(obj_entry), 1, fp) != 1) break;
        ...
        for (int c = 0; c < obj_entry.num_contents; c++) {
            struct copyover_obj_content cont_entry;
            if (fread(&cont_entry, sizeof(cont_entry), 1, fp) != 1) break;
    ...
    for (i = 0; i < header.num_rooms; i++) {
        if (fread(&room_entry, sizeof(room_entry), 1, fp) != 1) break;
    ...
    for (d = descriptor_list; d; d = d->next) {
        ...
        if (ch->specials.copyover_fighting_type == 1) {
            ...
            if (target && IS_NPC(target) && !IS_FIGHTING(ch)) {
                ...
                for (f = ch->followers; f; f = f->next) {
                    if (IS_NPC(f->follower) &&
                        ...
                        logit(LOG_STATUS, "copyover: pet %s joins combat vs %s",
            ...
            if (target && !IS_FIGHTING(ch)) {
                ...
                if (!IS_FIGHTING(target))
                    set_fighting(target, ch);
                logit(LOG_STATUS, "copyover: restored pvp %s vs %s",
                      GET_NAME(ch), GET_NAME(target));
            }
        }
