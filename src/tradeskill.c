...
void event_fish_check(P_char ch, P_char victim, P_obj, void *data)
{
  ...
  if (!ch->desc || IS_FIGHTING(ch) || IS_DESTROYING(ch) || (ch->in_room != fdata->room) || !MIN_POS(ch, POS_STANDING + STAT_NORMAL) || IS_SET(ch->specials.affected_by, AFF_HIDE) || IS_IMMOBILE(ch) || !IS_AWAKE(ch) || IS_STUNNED(ch) || IS_CASTING(ch))
  {
  ...
  {
    if (i->connected != CON_PLAYING || ch == i->character || i->character->following == ch || world[i->character->in_room].zone != world[ch->in_room].zone || ch->in_room == i->character->in_room || ch->in_room == real_room(i->character->specials.was_in_room) || real_room(ch->specials.was_in_room) == i->character->in_room)
    {
      continue;
    }
...
void event_mine_check(P_char ch, P_char victim, P_obj, void *data)
{
  ...
  if (!ch->desc || IS_FIGHTING(ch) || IS_DESTROYING(ch) || (ch->in_room != mdata->room) || !MIN_POS(ch, POS_STANDING + STAT_NORMAL) || IS_SET(ch->specials.affected_by, AFF_HIDE) || IS_IMMOBILE(ch) || !IS_AWAKE(ch) || IS_STUNNED(ch) || IS_CASTING(ch) || IS_AFFECTED2(ch, AFF2_CASTING))
  {
...
bool invalid_mine_room(int rroom_id)
{
  if (IS_ROOM(rroom_id, ROOM_PRIVATE) || PRIVATE_ZONE(rroom_id)
      ...
      || world[rroom_id].dir_option[DIR_DOWN] || IS_WATER_ROOM(rroom_id) || world[rroom_id].sector_type == SECT_MOUNTAIN || world[rroom_id].sector_type == SECT_ROAD || world[rroom_id].sector_type == SECT_UNDRWLD_MOUNTAIN || world[rroom_id].sector_type == SECT_UNDRWLD_NOGROUND || world[rroom_id].sector_type == SECT_UNDRWLD_NOSWIM || world[rroom_id].sector_type == SECT_UNDRWLD_WATER || world[rroom_id].sector_type == SECT_UNDRWLD_INSIDE || world[rroom_id].sector_type == SECT_UNDRWLD_CITY || world[rroom_id].sector_type == SECT_OCEAN || world[rroom_id].sector_type == SECT_INSIDE || world[rroom_id].sector_type == SECT_CASTLE || world[rroom_id].sector_type == SECT_CASTLE_WALL || world[rroom_id].sector_type == SECT_CASTLE_GATE)
    return TRUE;
...
int learn_recipe(P_obj obj, P_char ch, int cmd, char *arg)
{
  ...
  if (IS_SET(tobj->wear_flags, ITEM_WEAR_HEAD) || IS_SET(tobj->wear_flags, ITEM_WEAR_BODY) || IS_SET(tobj->wear_flags, ITEM_WEAR_ARMS) || IS_SET(tobj->wear_flags, ITEM_WEAR_FEET) || IS_SET(tobj->wear_flags, ITEM_WEAR_SHIELD) || IS_SET(tobj->wear_flags, ITEM_WIELD) || IS_SET(tobj->wear_flags, ITEM_WEAR_LEGS) || IS_SET(tobj->wear_flags, ITEM_WEAR_HANDS))
  {
    forge = TRUE;
  }
  if (IS_SET(tobj->wear_flags, ITEM_WEAR_ABOUT) || IS_SET(tobj->wear_flags, ITEM_WEAR_WAIST) || IS_SET(tobj->wear_flags, ITEM_WEAR_EARRING) || IS_SET(tobj->wear_flags, ITEM_WEAR_NECK) || IS_SET(tobj->wear_flags, ITEM_WEAR_WRIST) || IS_SET(tobj->wear_flags, ITEM_WEAR_FINGER) || IS_SET(tobj->wear_flags, ITEM_WEAR_EYES) || IS_SET(tobj->wear_flags, ITEM_WEAR_QUIVER) || IS_SET(tobj->wear_flags, ITEM_WEAR_TAIL) || IS_SET(tobj->wear_flags, ITEM_WEAR_NOSE) || IS_SET(tobj->wear_flags, ITEM_WEAR_HORN) || IS_SET(tobj->wear_flags, ITEM_WEAR_FACE))
  {
    craft = TRUE;
  }
...
int itemvalue(P_obj obj)
{
  ...
  while (i < MAX_OBJ_AFFECT)
  {
    ...
    if ((obj->affected[i].location == APPLY_STR) || (obj->affected[i].location == APPLY_DEX) || (obj->affected[i].location == APPLY_INT) || (obj->affected[i].location == APPLY_WIS) || (obj->affected[i].location == APPLY_CON) || (obj->affected[i].location == APPLY_AGI))
    {
    ...
    if ((obj->affected[i].location == APPLY_AGI_RACE) || (obj->affected[i].location == APPLY_STR_RACE) || (obj->affected[i].location == APPLY_CON_RACE) || (obj->affected[i].location == APPLY_INT_RACE) || (obj->affected[i].location == APPLY_WIS_RACE) || (obj->affected[i].location == APPLY_CHA_RACE) || (obj->affected[i].location == APPLY_DEX_RACE))
    {
    ...
    if ((obj->affected[i].location == APPLY_SAVING_PARA) || (obj->affected[i].location == APPLY_SAVING_ROD) || (obj->affected[i].location == APPLY_SAVING_FEAR) || (obj->affected[i].location == APPLY_SAVING_BREATH) || (obj->affected[i].location == APPLY_SAVING_SPELL))
    {
    ...
    if ((obj->affected[i].location == APPLY_STR_MAX) || (obj->affected[i].location == APPLY_DEX_MAX) || (obj->affected[i].location == APPLY_INT_MAX) || (obj->affected[i].location == APPLY_WIS_MAX) || (obj->affected[i].location == APPLY_CON_MAX) || (obj->affected[i].location == APPLY_CHA_MAX) || (obj->affected[i].location == APPLY_AGI_MAX) || (obj->affected[i].location == APPLY_POW_MAX) || (obj->affected[i].location == APPLY_LUCK_MAX))
    {
  ...
  if ((!CAN_WEAR(obj, ITEM_TAKE) && obj->type == ITEM_TELEPORT) || obj->type == ITEM_KEY || obj->type == ITEM_SWITCH || obj->type == ITEM_VEHICLE || obj->type == ITEM_SHIP || obj->type == ITEM_STORAGE)
  {
    if (workingvalue != 1)
    {
...
int get_ival_from_proc(obj_proc_type proc)
{
  ...
  if (proc == teleporting_map_pool || proc == storage_locker_obj_hook || proc == guildhall_door || proc == guildhall_heartstone || proc == tharnrifts_portal || proc == olympus_portal || proc == vecna_deathportal || proc == portal_door || proc == portal_wormhole || proc == verzanan_portal || proc == newbie_portal || proc == no_kill_priest_obj || proc == magic_pool || proc == moonstone_fragment || proc == treasure_chest || proc == cards_object || proc == artifact_monolith || proc == burbul_map_obj || proc == chyron_search_obj || proc == blood_stains || proc == ice_shattered_bits || proc == tracks || proc == frost_beacon || proc == ice_block || proc == charon_ship || proc == moonstone || proc == nexus || proc == pesky_imp_chest || proc == tower_summoning || proc == shabo_trap_north || proc == shabo_trap_north_two || proc == shabo_trap_south || shabo_trap_south_two || proc == shabo_trap_up || proc == shabo_trap_down || proc == shabo_trap_up_two || proc == slot_machine || proc == unspec_altar || proc == epic_stone || proc == stat_pool_str || proc == stat_pool_dex || proc == stat_pool_agi || proc == stat_pool_con || proc == stat_pool_pow || proc == stat_pool_int || proc == stat_pool_wis || proc == stat_pool_cha || proc == stat_pool_luc || proc == spell_pool || proc == druid_spring || proc == blighter_pond || proc == flying_citadel || proc == trap_razor_hooks || proc == trap_tower1_para || proc == trap_tower2_sleep || proc == hoa_plat || proc == vecna_deathportal || proc == vecna_deathaltar || proc == vecna_stonemist || proc == vecna_ghosthands || proc == vecna_torturerroom || proc == vecna_gorge || proc == mob_vecna_procs || proc == arenaobj_proc || proc == burn_touch_obj || proc == drowcrusher || proc == hewards_mystical_organ || proc == mir_fire || proc == board || proc == random_tomb || proc == random_glass || proc == random_slab || proc == refreshing_fountain || proc == magical_fountain || proc == changelog || proc == wall_generic || proc == huntsman_ward || proc == item_switch || proc == clock_tower || proc == verzanan_portal || proc == die_roller || proc == elfgate || proc == guildwindow || proc == guildhome || proc == automaton_lever || proc == illithid_teleport_veil || proc == teleporting_pool || proc == llyms_altar || proc == newbie_sign1 || proc == newbie_sign2 || proc == vareena_statue || proc == wh_corpse_decay || proc == dragon_heart_decay || proc == ravenloft_bell || proc == toe_chamber_switch || proc == flesh_golem_repop || proc == unmulti_altar || proc == jubilex_grid_mob_generator || proc == eth2_tree_obj || proc == magic_deck || proc == blackjack_table || proc == toe_chamber_switch || proc == drowcrusher)
  {
    return -100000;
  }
